...
function PricesLoadingScreen({ text }: { text: string }) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f8f6f1] px-5">
      <div className="text-center">
        <p className="font-tenor text-[34px] tracking-[-0.03em] text-black">
          LABRITY
        </p>

        <p className="mt-4 font-montserrat text-[10px] uppercase tracking-[0.28em] text-black/35">
          {text}
        </p>
      </div>
    </main>
...
export default function PricesPage() {
  ...
  const content = useMemo(() => prices, [prices]);
  ...
  if (!content) {
    return <PricesLoadingScreen text={fallbackData.ui.loading} />;
  }
